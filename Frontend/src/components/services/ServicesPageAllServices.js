  ...
  const services = useMemo(
    () => [
      ...
      },
      {
        ...
        imgWebp:
          "/assets/services/stock-photo-engineers-are-helping-to-design-work-on-blueprints-and-collaborate-on-structural-analyzing-of-1724713963 1.png",
        img:
          "/assets/services/stock-photo-engineers-are-helping-to-design-work-on-blueprints-and-collaborate-on-structural-analyzing-of-1724713963 1.png",
      },
    ],
    ...
    <section className="w-full bg-white py-[70px]" aria-labelledby="all-services-heading">
      <div className="max-w-[1250px] mx-auto px-[20px]">
        ...
        <p className="text-center text-[#4a5c7a] font-['Open_Sans'] text-[16px] leading-[1.7] max-w-[980px] mx-auto mt-[10px] mb-[35px]">
          We deliver a wide range of advanced scaffolding and access solutions, supported by a trained workforce and engineered
          design. Our services adapt to projects of all sizes and complexities — across construction, infrastructure, restoration,
          and events.
        </p>

        <div
          className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-10 auto-rows-fr"
          role="list"
          aria-label="Scaffolding and shoring services list"
        >
          {services.map((s, idx) => (
            <article
              key={`${s.title}-${idx}`}
            ...
            >
              ...
              <div className="p-[22px] flex flex-col flex-1">
                <h3 className="font-[Rajdhani] text-[#214f9b] text-[22px] font-[700] mb-[10px]">{s.title}</h3>

                <p className="font-['Open_Sans'] text-[#4a5c7a] text-[15px] leading-[1.7] flex-1">{s.desc}</p>
